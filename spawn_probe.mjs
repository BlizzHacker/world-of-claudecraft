...
for(let i=0;i<36;i++){
  const ui=await p.evaluate(()=>!!document.getElementById('ui'));
  if(ui) break;
  await new Promise(r=>setTimeout(r,5000));
}
await new Promise(r=>setTimeout(r,20000));
// dismiss camera dialog if present (click Confirm)
await p.evaluate(()=>{
  const btn=[...document.querySelectorAll('button')].find(b=>/confirm/i.test(b.textContent));
  btn?.click();
});
await new Promise(r=>setTimeout(r,3000));
// open chat and spawn mobs
...
const chat = await p.evaluate(()=>{
  const el=document.querySelector('#chat-log, .chat-log, #chat-messages');
  return el ? el.textContent.slice(-400) : 'NO_CHAT_EL';
});
console.log('chat tail:', JSON.stringify(chat).slice(0,500));
