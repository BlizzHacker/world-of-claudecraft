...
import { MAX_CHAT_MESSAGE_LEN, type PetState, type PlayerMeta, Sim } from '../src/sim/sim';
...
export class GameServer {
  ...
  join(
    ...
    this.send(session, {
      t: 'events',
      list: [{ type: 'log', text: `${name} has entered Cryptic Realm.`, color: '#ffd100' }],
    });
  ...
  private dispatchMessage(
    ...
    switch (command) {
      ...
      case 'removeProp':
      case 'setPropMeta':
        // Admin/mod world builder (ArcForge). Async (DB write + role check); the sim
        // entity change syncs to all clients on the next snapshot tick regardless.
