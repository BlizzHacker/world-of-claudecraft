...
/** POST /admin/api/grant-gm: grant or revoke in-game GM privileges by character name. */
async function grantGmHandler(ctx: Ctx): Promise<void> {
  const body = await readBody(ctx.req);
  const name = typeof body.character === 'string' ? body.character.trim() : '';
  const grant = body.revoke !== true;
  if (!name) return fail(ctx.res, 400, 'character name required');
  const updated = await setCharacterGmByName(name, grant);
  if (updated === 0) return fail(ctx.res, 404, 'no such character on this realm');
  ok(ctx.res, { character: name, is_gm: grant, updated });
}
...
export const routes: RouteDef[] = [
  ...
  {
    method: 'POST',
    path: '/admin/api/grant-gm',
    surface: 'admin',
    middleware: [requireAdmin],
    meta: ADMIN_META,
    handler: grantGmHandler,
  },
