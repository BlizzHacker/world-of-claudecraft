...
export function buildCharacterList(
  ...
  return {
    ...
    characters: chars.map((c) => {
      // Fork realms: every character renders through its realm's visual registry
      // (realm hero bodies), with the durance tester entitlement pinned so the
      // review account always sees a stable body.
      const charRealm = process.env.CR_REALM_ID ?? REALM;
      const appearance = isDuranceTesterCharacter(c.name, charRealm)
        ? { realmHeroId: null, visualKey: 'realm_infernal_durance_humanoid' as const }
        : resolveRealmCharacterVisual(charRealm, c.class, c.state?.realmHeroId);
      return {
      id: c.id,
      name: c.name,
      class: c.class,
