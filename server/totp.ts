...
//
// This fork ALSO keeps the original Cryptic Realm 2FA surface (totpCode /
// verifyTotpCode / otpauthUrl / generateTotpSecret) that the admin, dashboard,
// and login paths still import, layered on the same primitives.
...
// Legacy fork constants (kept as-is for the original TOTP surface).
...
// Legacy fork API: mint a base32 secret (defaults to 20 bytes / 160 bits).
...
// Legacy fork API: 6-digit code for a secret at a given time. Kept so callers
// and tests that used the original signature keep working.
...
// Legacy fork API: boolean verify around the current time. The admin, dashboard,
// and login paths still call this signature.
...
// Legacy fork API: object-arg otpauth URL builder, defaulting the issuer to the
// Cryptic Realm brand. Kept for the dashboard/user 2FA setup paths.
