...
// when CR_DISABLE_ADMIN_UPDATE=1 is set — useful for shared deploys where you
...
const LOG_PATH = process.env.CR_LOG_FILE ?? '/var/log/cr-update.log';
const MAINT_FLAG = path.join(REPO_ROOT, '.maintenance');
const DISABLED = (process.env.CR_DISABLE_ADMIN_UPDATE ?? '').trim() === '1';
...
function spawnUpdate(opts: { warnSeconds?: number; branch?: string }): RunResult {
  const env = {
    ...process.env,
    CR_WARN_SECONDS: String(opts.warnSeconds ?? 300),
    CR_BRANCH: opts.branch ?? 'master',
    CR_LOG_FILE: LOG_PATH,
  };
...
export async function maybeHandleAdminUpdate(
  ...
  if (pathname === '/admin/api/update' && req.method === 'POST') {
    if (DISABLED) {
      fail(res, 403, 'admin update disabled (CR_DISABLE_ADMIN_UPDATE=1)');
      return true;
    }
