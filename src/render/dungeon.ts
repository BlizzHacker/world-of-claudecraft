...
export class DungeonInteriors {
  ...
  private placeFloor(p: Placements, layout: DungeonLayout, variant: Variant): void {
    ...
    if (layout.doorway?.front) {
      ...
      for (let z = layout.zMin - 2; z >= layout.zMin - (DELVE_MODULE_GAP + 2); z -= FLOOR_CELL) {
        for (let x = frontX - doorHw; x <= frontX + doorHw; x += FLOOR_CELL) {
  ...
  private placeWalls(
    ...
    const doorHw = layout.doorway?.hw ?? DUNGEON_DOORWAY_HW;
    for (const end of [
      { z: layout.zMin, ry: 0, open: layout.doorway?.front ?? false, doorX: layout.doorway?.frontX ?? 0 },
      { z: layout.zMax, ry: Math.PI, open: layout.doorway?.back ?? false, doorX: layout.doorway?.backX ?? 0 },
    ]) {
      const target = arenaWalls
        ? end.z === layout.zMin
      ...
      for (let x = -endWallHw + 4; x <= endWallHw - 4; x += 8) {
        if (end.open && Math.abs(x - end.doorX) < doorHw + 4) continue; // leave the (off-centre) doorway open
