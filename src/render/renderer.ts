...
import { targetIntensity, targetIntensityFromValues } from './travel_speed_fx';
...
export class Renderer {
  ...
  showNameplates = true;
  // developer-badge display toggle (Interface options); nameplate glyph + outline
  showDevBadges = true;
  ...
  private createView(e: Entity): void {
    ...
    if (
      ...
      // ENTRANCE-type marker (waypoint pylon / town portal), else an exit.
      const entering = e.templateId === 'waypoint' || e.templateId === 'town_portal';
      ...
      // First-person part preservation is per-entity (the local player), so a
      // visual built with it can't be shared from the pool — only pool others.
      const wantsFirstPerson = e.id === this.sim.playerId;
      visualPoolKey = wantsFirstPerson ? null : this.visualPoolKeyFor(e);
  ...
  private updateAmbience(px: number, camY: number, dt: number): void {
    ...
    if (!inPractice && isInteriorPos(px)) {
  ...
  private updateCamera(selfPos: THREE.Vector3, dt: number): void {
    ...
    if (this.selfSubmerged && Number.isFinite(swimWaterLevel)) {
      cy = Math.min(cy, swimWaterLevel - UNDERWATER_CAMERA_DIP);
    }
