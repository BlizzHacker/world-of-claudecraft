...
export function buildProps(seed: number, delveLabel?: (delveId: string) => string): PropsResult {
  ...
  // A themed realm (infernal) scales building FOOTPRINTS (w/d) via worldTheme;
  // scale their HEIGHT by the same factor so a wider building is not a squashed
  // slab. Vanilla realms (buildingScale 1) and authored assetId placements get
  // vScale 1 and the original placement.
  const themeBuildingScale = getActiveRealm().worldTheme?.buildingScale ?? 1;
  // A wider building spans more (sloped) terrain, so placing its base at the
  // CENTRE's ground height leaves uphill corners buried. Sit the base at the
  // HIGHEST ground point under the footprint (with a small sink so the walls
  // still meet the dirt). Vanilla buildings keep the original centre placement.
  const footBaseY = (b: { x: number; z: number; w: number; d: number; rot: number }): number => {
    if (themeBuildingScale <= 1) return ground(b.x, b.z);
    const hw = b.w / 2;
    const hd = b.d / 2;
    ...
    for (const [lx, lz] of [
      ...
      const wx = b.x + lx * c - lz * sn;
      const wz = b.z + lx * sn + lz * c;
      maxY = Math.max(maxY, ground(wx, wz));
    }
    return maxY - 0.28; // small sink so the walls meet the dirt, not float
  };
  for (const b of activeContent.props.buildings) {
    const vScale = b.assetId ? 1 : themeBuildingScale;
    const y = b.assetId ? ground(b.x, b.z) : footBaseY(b);
    ...
    const roofY = y + buildingCameraHeight(b) * vScale;
    if (b.kind === 'chapel') {
      ...
      addParts(gTower, 'bellTower', {
        ...
        scale: [
          ...
          (CHAPEL_TOWER.height * vScale) / tower.size.y,
      ...
      addParts(gHall, 'house3', {
        ...
        scale: [
          ...
          (CHAPEL_HALL.height * vScale) / hall.size.y,
