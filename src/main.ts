...
// CR overlay: the homepage cinematic <video> element, kept so it can be paused
// when the game becomes active (set by initHomepageTrailer).
...
type GameRuntime = {
  ...
  voice: typeof import('./game/voice').voice;
  sfx: typeof import('./game/sfx').sfx;
...
async function startGame(
  ...
  function frame(now: number): void {
    ...
    try {
      ...
      // sim ticks and renderer.sync runs this frame. No-op in solo play.
      coopController?.frame(frameDtMs);
    } finally {
      perf.finishTrace('input.coop', traceStart, 'frameDtMs', frameDtMs);
    }
...
function refreshOfflineSkins(cls: PlayerClass): void {
  ...
  if (realmClassPresentation(cls)?.assetUrl || !CREATION_SKIN_PRESETS) {
...
function renderClassDetails(
  ...
  if (characterPreview) {
    ...
    else if (currentlyRenderedClass[panelId] !== className) showClassPreview(className);
  }
...
function wireStartScreens(): void {
  ...
  // On a logged-out entry, the desktop browser login page also reveals the in-app
  // login panel over the logged-out chrome. Skipped when a session was restored
  // (api.token set) or the Discord chooser is showing (parkedDiscordChoice).
  if (parkedDiscordChoice || api.token) {
    // logged in, or the Discord chooser owns the screen, nothing to surface here
  } else {
    enterLoggedOutChrome();
