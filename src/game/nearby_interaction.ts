...
export function tryNearbyInteraction(
  ...
  if (bestObject !== null) {
    ...
    if (object.templateId === 'dungeon_door' && object.dungeonId) {
      ...
      object.templateId === 'building_exit' ||
      object.templateId === 'waypoint' ||
      object.templateId === 'town_portal'
    ) {
      // Building interior exit door, waypoint pylon, town portal: server-authoritative
      // object interactions. Route through world.interact() (the server sim.interact)
  ...
  // building's own +z face (BUILDING_DOORS registry). No other arm claimed the
  // press, so if the player stands in a door area, send interact() and let the
  // sim (buildingDoorNear inside sim.interact, then enterInterior) do the entry
  // authoritatively, online and offline alike.
  if (buildingDoorNear(player.pos.x, player.pos.z)) {
    world.interact();
    return true;
  }
