...
function spawnOneDelveModule(
  ...
  // Drowned Litany baptistry room has its own scripted init (v0.20).
  ...
  if (baptistry) {
    ...
    for (const spawn of spawnSet.spawns) {
      ...
      // belligerent_dead affix (v0.20): tougher grave-silt bulwarks.
  ...
  // Drowned Litany finale (v0.20): arm the boss encounter state on the last room.
  else if (isFinale && run.delveId === 'drowned_litany') initDrownedLitanyBossState(run);
}
