...
function staticWorldColliders(seed: number): Collider[] {
  ...
  // Themed realms scale building height (render/props.ts reads the same theme);
  // mirror the scale here so the camera-hide top matches the taller mesh.
  // Vanilla realms (and authored assetId placements) use scale 1.
  const bScale = getActiveRealm().worldTheme?.buildingScale ?? 1;
  for (const b of PROPS.buildings) {
    const hScale = b.assetId ? 1 : bScale;
    if (b.kind === 'chapel' && b.assetId === undefined) {
      ...
      out.push({
        ...
        rot: b.rot,
        cameraTopY: topY(seed, b.x, b.z, buildingCameraHeight(b) * hScale),
      });
      ...
      out.push({
        ...
        rot: b.rot,
        cameraTopY: topY(seed, hx, hz, (CHAPEL_HALL_ROOF_TOP + 0.2) * hScale),
    ...
    const cameraTopY = isEastbrookGrandArmoury(b)
      ...
      : topY(seed, b.x, b.z, buildingCameraHeight(b) * hScale);
    out.push({
      type: 'obb',
      x: b.x,
      z: b.z,
