...
export function castAbility(
  ...
  // Upstream haste (spellHasteMult) and the realm's gcdMult both apply: haste speeds
  // the whole rotation, the D2 realms then collapse it further. 0.15 is the livelock floor.
  const gcd = Math.max(0.15, (ctx.playerGcdFor(meta.cls) / spellHasteMult(p)) * gcdMult);
