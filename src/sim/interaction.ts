...
export function lootCorpse(
  ...
  if (mob.kind === 'player' && mob.hardcoreCorpse) {
    ...
    for (const sl of [...mob.loot.items]) {
      for (let i = 0; i < sl.count; i++) ctx.addItem(sl.itemId, 1, meta.entityId);
      sl.count = 0;
    }
...
export function interact(
  ...
  if (p.targetId !== null) {
    ...
    if (target && dist2d(p.pos, target.pos) <= INTERACT_RANGE + 2) {
      // ArcForge prop with metadata speaks its dialogue and carries optional audio.
      if (target.kind === 'object' && target.templateId?.startsWith('prop:')) {
        const m = ctx.propMetaByEnt.get(target.id);
      ...
      if (target.kind === 'player' && target.hardcoreCorpse && target.lootable) {
        lootCorpse(ctx, target.id, p.id);
        return;
      }
  ...
  ctx.playerGrid.forEachInRadius(p.pos.x, p.pos.z, INTERACT_RANGE, (e, d2) => {
    if (
      e.id !== p.id &&
      e.kind === 'player' &&
      e.hardcoreCorpse &&
