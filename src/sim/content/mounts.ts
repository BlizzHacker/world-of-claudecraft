...
// ---------------------------------------------------------------------------
// Cryptic Realm legacy aura/item mounts. Predates the MountKey catalog above:
// bridle ITEMS toggle a `mount_<id>` buff_speed aura, the renderer swaps the
// body to the mount rig while the aura is up, and the Thornwheel Derby seats
// its loaner kart through the same aura. Live characters own these bridle
// items, so the layer stays wire- and save-compatible alongside the catalog.
// ---------------------------------------------------------------------------

export interface LegacyMountDef {
  id: string;
  name: string;
...
export const LEGACY_MOUNTS: Record<string, LegacyMountDef> = {
  forest_stag: {
    id: 'forest_stag',
    name: 'Forest Stag',
    speedMult: 1.6,
...
export function mountForAuraId(auraId: string): LegacyMountDef | null {
  if (!auraId.startsWith(MOUNT_AURA_PREFIX)) return null;
  return LEGACY_MOUNTS[auraId.slice(MOUNT_AURA_PREFIX.length)] ?? null;
}
...
export const MOUNT_ITEMS: Record<string, ItemDef> = {
  ...
  mount_emerald_wyrm: {
    ...
    sellValue: 2500000,
    buyValue: 10000000, // 1000g — the $CR settlement path lands next (housing pattern)
    stackSize: 1,
  },
};