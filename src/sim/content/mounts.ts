...
// Mounts: rideable speed buffs bought as items (Cryptic Realm feature; upstream
// has no mount system). Using the item toggles a `mount_<id>` buff_speed aura
// (moveSpeedMult already honors buff_speed), and the renderer swaps the body to
// the mount rig while the aura is up — the Ghost Wolf / druid-form pattern.
// Taking damage dismounts (breaksOnDamage), classic-mount style. The item is a
// permanent bridle: never consumed, tradeable (a real economy good).
