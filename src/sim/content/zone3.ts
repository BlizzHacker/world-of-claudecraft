...
export const ZONE3_MOBS: Record<string, MobTemplate> = {
  ...
  // ---------------------------------------------------------------------
  // Highland backcountry beasts (levels 14-19). No curated body by design - see
  // the note on the Vale pack in content/zone1.ts. The rendered quadruped is
  // hash-picked from the active realm's staged creature roster, so names and
  // flavour never name a species, a hide, or a weapon-shaped body part.
  // ---------------------------------------------------------------------
  highwind_warbeast: {
  ...
  frostline_scourge: {
    ...
    moveSpeed: 8,
    aggroRadius: 12,
    slowStrike: { chance: 0.25, mult: 1.3, duration: 8, name: 'Numbing Blow', school: 'frost' },
    loot: [
      { copper: 88, chance: 1 },
      { itemId: 'healing_potion', chance: 0.08 },
    ],
...
// Highland backcountry beast packs (levels 14-19). Own export, spread at the
// TAIL of the merged CAMPS array (data.ts) so these draws never perturb the
// existing camps' deterministic placement. Centres sit off the passes and clear
// of the ogre, kobold, elemental, wyrmcult and revenant pull rings.
export const ZONE3_WILDS_CAMPS: CampDef[] = [
  { mobId: 'highwind_warbeast', center: { x: 60, z: 560 }, radius: 18, count: 5 },
  { mobId: 'cairnfell_render', center: { x: -95, z: 620 }, radius: 16, count: 5 },
  { mobId: 'scarpfall_fury', center: { x: 110, z: 680 }, radius: 18, count: 5 },
  { mobId: 'frostline_scourge', center: { x: -125, z: 780 }, radius: 16, count: 5 },
  { mobId: 'stormfell_ravener', center: { x: 78, z: 878 }, radius: 16, count: 5 },
  { mobId: 'talusrift_devourer', center: { x: -95, z: 872 }, radius: 16, count: 5 },
];
