...
export function buildOptionsMenu(opts: { bugReportAvailable: boolean }): OptionsMenuEntry[] {
  ...
  entries.push({ labelKey: 'hud.options.customization', action: { kind: 'customization' } });
  entries.push({ labelKey: 'hud.options.mods', action: { kind: 'mods' } });
  entries.push({ labelKey: 'hud.options.arcforge', action: { kind: 'arcforge' } });
