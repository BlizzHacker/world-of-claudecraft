...
export class Hud {
  ...
  update(): void {
    ...
    if (mediumHud) {
      ...
      crypticMusic.update(zone, musicCombat);
  ...
  private renderDelveBoard(focus = false): void {
    ...
    if (tab === 'shop') {
      ...
      el.querySelector('[data-delve-enter]')?.addEventListener('click', () => {
        ...
        this.closeDelveBoard();
        // Show the SAME branded loading screen used at world entry, and preload EVERY
        // monster GLB + the dungeon kit before dropping the player in, so the crawl
        // doesn't hitch as each heavy Hellmaw body first scrolls into view. On slow/
  ...
  private openSelfContextMenu(x: number, y: number, opener: HTMLElement | null = null): void {
    ...
    let html = `<div class="ctx-title ctx-title-player">${portraitChipHtml({ cls: this.sim.cfg.playerClass, skin: this.sim.player.skin ?? 0, name: this.sim.player.name, variant: 'sm', catalog: this.sim.player.skinCatalog, visualKey: visualKeyFor(this.sim.player) })}<span class="ctx-title-name">${esc(this.sim.player.name)}</span></div>`;
  ...
  private renderOptions(): void {
    ...
    const syncMusicLabel = () => {
      ...
      const state = on ? t('hud.options.on') : t('hud.options.off');
      const track = on ? music.nowPlaying() : null;
    ...
    musicBtn.addEventListener('click', () => {
      audio.click();
      // Drive BOTH engines like the Audio options switch: off means silence.
      const on = !(music.enabled || crypticMusic.enabled);
      music.setEnabled(on);
