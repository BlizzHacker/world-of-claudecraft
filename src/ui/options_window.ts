...
export class OptionsWindow {
  ...
  private mobileShellDeps(): MobileShellDeps {
    return {
      ...
      onCustomization: () => this.runQuickAction('customization'),
      onMods: () => this.runQuickAction('mods'),
      onArcForge: () => this.runQuickAction('arcforge'),
  ...
  private renderBugReport(detail: HTMLElement): void {
    ...
    // capture() is async (the encode must not block the options window); the
    // panel renders immediately and the preview fills in when the frame lands.
    ...
    let includeShot = false;
    {
      ...
      void hooks.capture().then((captured) => {
        shot = captured;
        if (!captured || !shotWrap.isConnected) return;
        img.src = captured;
        includeShot = true;
