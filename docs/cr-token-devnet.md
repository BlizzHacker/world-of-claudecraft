# \ Token — Devnet Deployment Record (2026-07-18)
...
| Mint authority | \ (deployer/controller) |
...
Keys: deployer keypair lives on CT172 \; a copy
...
Boot proof: \"ECONOMY: Solana chain enabled (solana-devnet)\" on apex and
exchange realms. Settlement adapter mints/reads via the spl-token CLI.
...
2. Split authorities: cold treasury (hardware/multisig), hot settlement key
   with a mint cap or transfer-from-treasury model instead of open mint.
