# Phase 25 QA: Verify Integrate Fishing for One-to-Four-Player Checkpoint

### QA Starter Prompt

~~~text
This is Phase 25 QA of Cryptic Realm Recovery and Modernization: Verify Integrate Fishing for One-to-Four-Player Checkpoint.
...
Goal: Independently verify every Phase 25 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-25-fishing-integration-checkpoint.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Run cross-player simultaneous timing/reconnect cases.
- Force a fishing smoke failure only in isolated stage/simulation and verify rollback before
...
- After PASS only, invoke Phase 04 automatic promotion for world fishing.
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 26 handoff.
