# Phase 21 QA: Verify Integrate Mounts and Flight for Release Checkpoint

### QA Starter Prompt

~~~text
This is Phase 21 QA of Cryptic Realm Recovery and Modernization: Verify Integrate Mounts and Flight for Release Checkpoint.
...
Goal: Independently verify every Phase 21 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-21-mount-flight-integration-checkpoint.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Prove target-neutral diffs and restored-fixture behavior.
- Promote only after PASS; force a flight/realm smoke failure only in isolated stage or
  simulation to prove rollback, then observe production and roll back on natural failure.
...
- After PASS only, invoke Phase 04 automatic promotion for mounts and flight.
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 22 handoff.
