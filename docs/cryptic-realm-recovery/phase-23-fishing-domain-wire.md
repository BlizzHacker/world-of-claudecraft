...
Model visible real-world fishing states while preserving deterministic server-authoritative catches across all hosts.
...
~~~text
This is Phase 23 of Cryptic Realm Recovery and Modernization: Implement World Fishing Domain and Wire.
...
Goal: Model visible real-world fishing states while preserving deterministic server-authoritative catches across all hosts.

STEP 0 - PRE-FLIGHT:
- Verify branch and git status; preserve unrelated/concurrent work; record phase-start commit and UTC timestamp.
- Confirm Phase 22 QA is complete.
...
Run separately:
- focused fishing domain/online tests
...
- feat(fishing): add world reel states
- feat(net): mirror fishing state
- test(fishing): cover reward authority

STEP 5 - ACCEPTANCE CRITERIA:
- [ ] World fishing has explicit deterministic states.
- [ ] Server decides catches/rewards.
- [ ] All hosts and reconnect agree.
- [ ] Arcade code cannot enter world reward path.
...
Report status, files, commits, validation, reviewers, QA artifacts, deferrals, feature flags, and Phase 23 QA handoff.

STOPPING RULES:
- Do not invent catch rates/rewards.
- Do not let renderer timing decide catches.
~~~
