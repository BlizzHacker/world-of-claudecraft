# Phase 48 QA: Verify Complete Final Hardening and Automatic Release

### QA Starter Prompt

~~~text
This is Phase 48 QA of Cryptic Realm Recovery and Modernization: Verify Complete Final Hardening and Automatic Release.
...
Goal: Independently verify every Phase 48 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-48-final-hardening-release.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Independently reproduce every gate and scenario evidence.
- Issue PASS before promotion, then verify all realms or prove rollback.
...
- stage-only rollback injection followed by production post-pass observation; never inject
  a failure into production
...
- After PASS only, invoke Phase 04 automatic promotion for complete Cryptic Realm recovery.
...
After all phases are complete, production healthy, and durable follow-ups external, ask:
"All phases are complete and green. OK to delete docs/cryptic-realm-recovery/ before final cleanup?"
Delete only this directory after confirmation. Never git add -A.

STEP 6 - FINAL RESPONSE FORMAT:
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and packet status.
...
- Rollback automatically on any HEAD/build/API/realm/console/error threshold failure.
~~~
