# Phase 15 QA: Verify Complete Co-op Baseline and Release Checkpoint

### QA Starter Prompt

~~~text
This is Phase 15 QA of Cryptic Realm Recovery and Modernization: Verify Complete Co-op Baseline and Release Checkpoint.
...
Goal: Independently verify every Phase 15 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-15-coop-integration-checkpoint.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Run the exact scenario matrix and record accounts, steps, expected/actual, verdict, evidence.
- Force one co-op smoke failure in isolated stage/simulation and prove rollback before
  promotion; never inject a failure into production.
...
- exact `6564b2bca90d5b0979994e6e76e27743a3aab2c1` duplicate-realm-entry regression across
  reload and realm switching before PASS
...
- After PASS only, invoke Phase 04 automatic promotion for co-op.
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 16 handoff.
