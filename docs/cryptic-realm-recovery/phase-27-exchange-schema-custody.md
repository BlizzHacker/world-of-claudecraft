...
Implement separate online transactional persistence and offline local-profile persistence for
the same Exchange state machine without a cross-boundary conversion path.
...
- Add a versioned local-only offline-profile escrow ledger with deterministic transitions,
  save isolation, crash-safe writes, and no SQL/network/import/export adapter.
- Guarantee old-data boot, DDL-twice, forward-compatible rollback, non-destructive recovery,
  concurrency/crash/retry conservation, and decisive offline/online isolation tests.

### Starter Prompt

~~~text
This is Phase 27 of Cryptic Realm Recovery and Modernization: Implement Exchange Schema, Escrow, and Currency Reservations.
...
Goal: Implement additive transactional persistence for item escrow, provenance, buyer reservations, fees, refunds, proceeds, leases, and audit.

STEP 0 - PRE-FLIGHT:
- Verify branch and git status; preserve unrelated/concurrent work; record phase-start commit and UTC timestamp.
- Confirm Phase 26 QA is complete.
...
- docs/cryptic-realm-recovery/phase-27-exchange-schema-custody.md
- config/cryptic-recovery/features.json if it exists
- docs/operations/cryptic-recovery-runbook.md if it exists
- server/db.ts and market DB helpers
- Phase 26 policy
- PostgreSQL official docs
- existing realm market isolation tests
...
- Add the versioned local-only offline-profile escrow ledger with deterministic transitions,
  save isolation, crash-safe writes, and no SQL/network/import/export adapter.
- Guarantee old-data boot, DDL-twice, forward-compatible rollback, non-destructive recovery,
  concurrency/crash/retry conservation, and decisive offline/online isolation tests.
...
Run separately:
- focused exchange DB/custody tests
- DDL twice and old-data fixtures
- concurrent workers and crash injection
- money/item conservation reconciliation
- offline save round-trip plus forbidden online import/export/sync tests
...
- feat(db): add exchange escrow ledger
- feat(exchange): reserve funds atomically
- test(exchange): prove item money conservation

STEP 5 - ACCEPTANCE CRITERIA:
- [ ] Item and money move exactly once under every outcome.
- [ ] Fees post only on successful settlement; failures/cancels refund exact reserved remainder.
- [ ] Leases/audit/reconciliation are indexed and idempotent.
- [ ] Old app/data compatibility and rollback canary pass.
- [ ] Offline ledger round-trips locally and cannot address or mutate any online identity,
  listing, escrow row, currency balance, or item.
...
Report status, files, commits, validation, reviewers, QA artifacts, deferrals, feature flags, and Phase 27 QA handoff.

STOPPING RULES:
- Do not hold custody only in character JSONB.
- Do not delete forward-created ledger/audit data on rollback.
- Do not add offline-to-online import, export, reconciliation, or settlement.
~~~
