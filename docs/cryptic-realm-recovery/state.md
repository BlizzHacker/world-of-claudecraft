...
|---|---|---|---|---|---|---|
| 2026-07-18 | Realm-integrated venues (supersedes arcade-window design of 33-36, 42-44) | 14ee55177, 42de90341, e68c6b32c, 458709efc + i18n follow-up | npm run gate green on branch except the recovery-manifest branch-pin (mainline-only by design); 12,625/12,627 vitest, tsc, parity goldens re-derived (terrain-height class only) | Thornwheel Derby (physical kart track, mount form-swap karts), Boarpit (KO brawls, jail-brawl hostility + duel 1hp clamp), war-table-only RTS board, fishing pinned at all 5 lakes, Eastbrook Homes premium ($CR deed entitlement seam, homes persistence) | Menu minigames deleted; housing arcade mode permanently disabled; $CR settlement callback is a documented seam (HOMEOWNER_CHARACTERS allowlist interim); phase QA packets for 33-36/42-44 not yet run against this superseding design | User: ff-merge codex/realm-integrated-minigames into codex/cryptic-recovery-program, then stage promotion |
