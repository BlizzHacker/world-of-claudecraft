# Phase 31 QA: Verify Implement Reusable Minigame Domain and Wire

### QA Starter Prompt

~~~text
This is Phase 31 QA of Cryptic Realm Recovery and Modernization: Verify Implement Reusable Minigame Domain and Wire.
...
Goal: Independently verify every Phase 31 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-31-minigame-domain-wire.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Compare Vale Cup golden behavior before/after.
- Trace `gauntlet_reference_session` to exact source ref `upstream/feature/gauntlet-event` at `196487c8688825d6831278191d3160e622142ec5`, verify its ledger disposition, and prove the default-off adapter exercises queue through cleanup without leaking game-specific rules into the shared lifecycle.
- Fuzz every lifecycle transition and disconnect order.
...
- `gauntlet_reference_session` adapter/scenario regression pinned to `196487c8688825d6831278191d3160e622142ec5`
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 32 handoff.
...
- Missing, renamed, always-on, or source-untraceable `gauntlet_reference_session` is FAIL.
~~~
