# Phase 44 QA: Verify Implement Housing Client and Release Checkpoint

### QA Starter Prompt

~~~text
This is Phase 44 QA of Cryptic Realm Recovery and Modernization: Verify Implement Housing Client and Release Checkpoint.
...
Goal: Independently verify every Phase 44 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-44-housing-client-checkpoint.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Run owner/guest/denied/deleted scenarios for all player counts.
- Force auth/privacy failure only in isolated stage/simulation and verify rollback before
  promotion; never inject an auth or privacy failure into production.
...
- After PASS only, invoke Phase 04 automatic promotion for housing.
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 45 handoff.
...
- Do not enable a quarantined slice during checkpoint.
- Do not use real wallet/funds in QA.
~~~
