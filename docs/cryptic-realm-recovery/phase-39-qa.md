# Phase 39 QA: Verify Implement Town RTS Client and One-to-Four-Player Integration

### QA Starter Prompt

~~~text
This is Phase 39 QA of Cryptic Realm Recovery and Modernization: Verify Implement Town RTS Client and One-to-Four-Player Integration.
...
Goal: Independently verify every Phase 39 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-39-rts-client-integration.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Run each player count and input mode in Eastbrook, then Highwatch.
- Compare preview and authoritative placement rejection.
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 40 handoff.
...
- Do not enable Highwatch on an Eastbrook failure.
- Do not let client previews bypass placement validation.
~~~
