# Phase 10 QA: Verify Shared Delivery Status, Contribution Website, and Foundation Checkpoint

### QA Starter Prompt

~~~text
This is Phase 10 QA of Cryptic Realm Recovery and Modernization: Verify Shared Delivery Status, Contribution Website, and Foundation Checkpoint.
...
Goal: Independently verify every Phase 10 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-10-contribution-website-checkpoint.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Compare rendered entries to real current PR fixtures.
- Compare public and authenticated operator projections from the same delivery records; prove
  each surface contains only its explicit allowlist and that operator authorization fails closed.
- Prove a bug-report-class fixture is absent from public status by default and can expose only
  a sanitizer-approved summary after explicit consent. Do not implement or exercise the
  Phase 45 submission, attachment, sanitizer, or GitHub issue adapter here.
- Verify dated fallback, cache recovery, staleness, and rate-limit behavior without exposing
  private payloads, credentials, internal identifiers, or raw failures.
- Run the full checkpoint promotion ordering and forced rollback proof only in isolated
  stage/simulation; production performs health observation and rolls back only on a naturally
  detected failure.
...
- After PASS only, invoke Phase 04 automatic promotion for foundation and contributions.
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 11 handoff.
...
- Fail if public or operator status exposes a raw payload, credential, private source mapping,
  internal-only failure, or non-consented bug-report summary.
- Do not add feature-specific bug-report behavior before Phase 45.
~~~
