...
Implement deterministic vertical movement, constraints, and recovery for explicitly flight-capable mounts.
...
~~~text
This is Phase 19 of Cryptic Realm Recovery and Modernization: Implement Authoritative Flight Domain and Wire.
...
Goal: Implement deterministic vertical movement, constraints, and recovery for explicitly flight-capable mounts.

STEP 0 - PRE-FLIGHT:
- Verify branch and git status; preserve unrelated/concurrent work; record phase-start commit and UTC timestamp.
- Confirm Phase 18 QA is complete.
...
- docs/cryptic-realm-recovery/phase-19-flight-domain-wire.md
...
Run separately:
- focused flight authority tests
...
- feat(mounts): add deterministic flight
- feat(net): validate flight state
- test(mounts): cover flight exploits

STEP 5 - ACCEPTANCE CRITERIA:
- [ ] Only capability-approved mounts fly.
- [ ] Server validates every altitude outcome and safely recovers invalid state.
- [ ] Host parity and canary wire tests pass.
- [ ] Emerald Wyrm capability is data-driven, not client-provided.
...
Report status, files, commits, validation, reviewers, QA artifacts, deferrals, feature flags, and Phase 19 QA handoff.

STOPPING RULES:
- Do not persist unrecoverable invalid altitude.
- Do not grant flight to all mounts.
~~~
