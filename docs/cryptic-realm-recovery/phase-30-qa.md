# Phase 30 QA: Verify Integrate Exchange for One-to-Four-Player Checkpoint

### QA Starter Prompt

~~~text
This is Phase 30 QA of Cryptic Realm Recovery and Modernization: Verify Integrate Exchange for One-to-Four-Player Checkpoint.
...
Goal: Independently verify every Phase 30 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-30-exchange-integration-checkpoint.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Run item and money reconciliation after every failure.
- Force settlement/health failure only in isolated stage/simulation and verify rollback
  before promotion; never inject a custody or health failure into production.
...
- After PASS only, invoke Phase 04 automatic promotion for Exchange Realm.
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 31 handoff.
