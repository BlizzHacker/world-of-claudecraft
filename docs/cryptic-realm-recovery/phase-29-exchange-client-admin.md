...
Build accessible Exchange-only UX and bounded operator reconciliation without enabling prohibited realm gameplay.
...
- Present projected inventory explicitly as escrow-backed and non-owning; clearly label
  Offline Exchange as permanently local/non-syncable and Online Exchange as server-backed;
  hide/disable combat, quests, mail, progression, and ordinary market controls.
...
~~~text
This is Phase 29 of Cryptic Realm Recovery and Modernization: Implement Exchange Player and Operator Experience.
...
Goal: Build accessible Exchange-only UX and bounded operator reconciliation without enabling prohibited realm gameplay.

STEP 0 - PRE-FLIGHT:
- Verify branch and git status; preserve unrelated/concurrent work; record phase-start commit and UTC timestamp.
- Confirm Phase 28 QA is complete.
...
- docs/cryptic-realm-recovery/phase-29-exchange-client-admin.md
- config/cryptic-recovery/features.json if it exists
- docs/operations/cryptic-recovery-runbook.md if it exists
- src/ui market modules
- src/admin
- Exchange IWorld state
- i18n
- mobile visual scripts
...
- Build listing/browse/quote/confirm/cancel/delivery UX showing provenance, source/destination realm, compatibility, reserved funds, price, fee, proceeds/refund rules, lease expiry, and final state.
- Present projected inventory explicitly as escrow-backed/non-owning, label the permanent
  offline/online economy boundary, and hide/disable prohibited gameplay controls.
...
Run separately:
- focused Exchange UI/admin tests
- npx vitest run tests/localization_fixes.test.ts
- desktop/controller/phone screenshots
- admin auth/redaction tests
- offline-local versus online-server authority copy and adapter tests
...
- feat(exchange): add trading workflow
- feat(admin): add exchange reconciliation
- test(exchange): cover accessible confirmations

STEP 5 - ACCEPTANCE CRITERIA:
- [ ] Players see full economic consequences before confirm.
- [ ] Projected inventory and prohibited gameplay are unambiguous.
- [ ] Offline UX never offers sync/import/export/online delivery and cannot be confused with
  server-backed listings or balances.
- [ ] Operator tools cannot directly forge custody/money.
- [ ] All locales/input/accessibility/error states pass.
...
Report status, files, commits, validation, reviewers, QA artifacts, deferrals, feature flags, and Phase 29 QA handoff.

STOPPING RULES:
- Do not add a client-only settlement shortcut.
- Do not expose owner account identifiers in player UI.
~~~
