# Phase 40 QA: Verify Implement Zombie Tower Defense Domain and Wire

### QA Starter Prompt

~~~text
This is Phase 40 QA of Cryptic Realm Recovery and Modernization: Verify Implement Zombie Tower Defense Domain and Wire.
...
Goal: Independently verify every Phase 40 deliverable and acceptance item before completion.
...
STEP 1 - LOAD CONTEXT:
Spawn Explore to summarize state, progress, inventory, checklist, phase-40-zombie-domain-wire.md, full diff, permanent guards, root AGENTS/CLAUDE and governing CLAUDE files. Return every deliverable/criterion, changed surface, flag, test, known issue, environment, and evidence gap.

STEP 2 - QA AUDIT:
Spawn fresh parallel agents.

Correctness:
- Fuzz routes, targets, upgrades, restarts, and max load.
- Verify no open-world or client-authoritative path.
...
Report verdict, finding/fix counts, SHAs, validations, environment, QA artifacts, cleanup, promotion/operation/rollback, flags, deferrals, and Phase 41 handoff.
