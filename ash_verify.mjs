...
console.log('enter:', await p.evaluate(()=>{
  ...
  row?.querySelector('.enter-world-btn')?.click(); return 'row-btn';
}));
for(let i=0;i<36;i++){
...
await new Promise(r=>setTimeout(r,2000));
// zoom in for a closer body shot
await p.mouse.move(800,450);
await p.mouse.wheel({deltaY:-600});
await new Promise(r=>setTimeout(r,1500));
// idle shot first
await p.screenshot({path:'/tmp/anim_idle1.png'});
await new Promise(r=>setTimeout(r,1200));
await p.screenshot({path:'/tmp/anim_idle2.png'});
// walk toward camera: hold S, capture 3 mid-stride frames
await p.keyboard.down('s');
await new Promise(r=>setTimeout(r,700));  await p.screenshot({path:'/tmp/anim_walk1.png'});
await new Promise(r=>setTimeout(r,450));  await p.screenshot({path:'/tmp/anim_walk2.png'});
await new Promise(r=>setTimeout(r,450));  await p.screenshot({path:'/tmp/anim_walk3.png'});
await p.keyboard.up('s');
await new Promise(r=>setTimeout(r,1500));
await p.screenshot({path:'/tmp/anim_after.png'});
await br.close();
console.log('done');
