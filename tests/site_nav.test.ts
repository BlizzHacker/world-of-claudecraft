...
describe('site and dashboard navigation', () => {
  ...
  it('keeps the public footer on Cryptic versioning and Diabl0 network links', () => {
    expect(packageJson.version).toBe('0.18.0-cr.1');
    expect(indexHtml).toContain('<div id="game-version">v0.18.0-cr.1</div>');
    expect(indexHtml).toContain('ClaudeCraft target v0.18.0');
