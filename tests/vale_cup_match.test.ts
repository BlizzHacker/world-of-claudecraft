...
import { MOBS, DUNGEON_X_THRESHOLD, GATHER_NODES } from '../src/sim/data';
...
describe('Vale Cup: determinism', () => {
  ...
  it('draws ZERO shared rng anywhere on the queue + match path (draw-value accounting)', () => {
    const script = (withCup: boolean): number[] => {
      ...
      // Keep the control actors inside the quiet stadium shell; the Cup run moves
      // them onto the pitch, so a default-spawn control would measure ambient aggro
      // differences instead of Vale Cup rng purity.
      const a = addAt(sim, 'warrior', 'Aleph', PITCH_CENTER.x, PITCH_CENTER.z);
      const b = addAt(sim, 'mage', 'Bet', PITCH_CENTER.x + 6, PITCH_CENTER.z);
      const values: number[] = [];
  ...
  it('the BOT path (practice: spawn, chase, kicks, shoulders, dives) also draws zero shared rng', () => {
    const script = (withCup: boolean): number[] => {
      ...
      const origin = vcPracticeOrigin(0);
      teleport(sim, sim.primaryId, PITCH_CENTER.x + origin.x, PITCH_CENTER.z + origin.z);
      const values: number[] = [];
      sim.rng.setObserver((v) => values.push(v));
