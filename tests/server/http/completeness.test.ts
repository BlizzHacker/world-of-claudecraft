...
// Prefix-delegated sub-dispatchers are scanned too: their exact-path /api arms sit
// behind main.ts `startsWith(...)` prefix arms, so they are legacy-served even
// though main.ts never spells the concrete paths (same blind-spot fix as the
// freshness gate's DISPATCHER_SOURCES; extend this list for any future
// prefix-delegated module).
