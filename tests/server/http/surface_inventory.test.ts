...
describe('surface inventory: content-type classification completeness', () => {
  ...
  it('maps REDIRECT to the Authentik start routes today', () => {
    const redirectPaths = SURFACE_INVENTORY.filter((r) => r.contentType === REDIRECT).map(
      (r) => r.path,
    );
    expect(redirectPaths).toEqual(['/api/oauth/authentik/link', '/api/oauth/authentik']);
  });
});
