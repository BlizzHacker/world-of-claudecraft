...
describe("i18n status summary: committed audit rollup cross-checks the full registry", () => {
  ...
  it("the per-locale rollup tallies to the registry totals and to keys-per-locale", () => {
    ...
    for (const lang of NON_EN) {
      ...
      expect(row.translated + row.pending + row.blocked, `${lang} rollup != key count`).toBe(registry.counts.keys);
      t += row.translated;
      p += row.pending;
      b += row.blocked;
    }
    expect([t, p, b]).toEqual([registry.counts.translated, registry.counts.pending, registry.counts.blocked]);
  });
    ...
    const derived = createHash("sha256")
      .update(
        Object.entries(registry.keys)
          .map(([ck, e]: [string, any]) => `${ck}${SEP}${e.enHash}${SEP}${e.placeholders.join(",")}`)
          .join("\n"),
        "utf8",
      )
      .digest("hex");
