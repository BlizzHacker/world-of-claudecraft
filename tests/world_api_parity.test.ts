...
//
// `IWorld` (src/world_api.ts, 228 members) is the ONE seam render/ui depend
...
// it lands the matching edit here in the SAME commit. The count pins (228 / 58 / 170)
...
// The 228 members of `interface IWorld`, in interface order (world_api.ts).
// Partition: 58 `data` + 170 `method` (read-returning + command-void + async).
...
export const IWORLD_MEMBERS = [
  ...
  { name: 'minigameArcadeState', kind: 'data' },
  { name: 'minigameZombieState', kind: 'data' },
  { name: 'minigameCreate', kind: 'method' },
  { name: 'minigameInvite', kind: 'method' },
  { name: 'minigameJoin', kind: 'method' },
  { name: 'minigameReady', kind: 'method' },
  { name: 'minigameAbort', kind: 'method' },
  { name: 'minigameClaim', kind: 'method' },
  { name: 'minigameZombieStart', kind: 'method' },
  { name: 'minigameZombieBuild', kind: 'method' },
  { name: 'minigameRaceInput', kind: 'method' },
  { name: 'minigameBrawlerInput', kind: 'method' },
  { name: 'minigameRtsBuild', kind: 'method' },
  { name: 'minigameRtsTrain', kind: 'method' },
  { name: 'minigameHousingPlace', kind: 'method' },
  ...
  // Downstream (Cryptic Realm) ArcForge world-builder commands (IWorldInteraction).
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  ...
  it('the full sorted member set is exactly the pinned 221', () => {
...
//   (4) their union, sorted, equals the pinned 228-name IWORLD_MEMBERS set (a member
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 26 facets', () => {
  ...
  it('the union of the 26 facets equals the pinned 228-member IWORLD_MEMBERS set', () => {
    const union = Object.values(FACET_MEMBER_ARRAYS).flatMap((arr) => [...arr]);
    expect(union.length, 'union size before dedup (catches a duplicated member)').toBe(228);
    expect(new Set(union).size, 'union size after dedup (catches a duplicated member)').toBe(228);
