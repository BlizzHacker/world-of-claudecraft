...
const MAC_DMG_RE = /cryptic-realm-\d+\.\d+\.\d+-mac-universal\.dmg/g;
const LINUX_APPIMAGE_RE = /cryptic-realm-\d+\.\d+\.\d+-linux-x86_64\.AppImage/g;
...
const WINDOWS_INSTALLER_RE = /cryptic-realm-\d+\.\d+\.\d+-win-x64\.exe/g;
...
const LEGACY_WINDOWS_INSTALLER_RE = /cryptic-realm-\d+\.\d+\.\d+-win\.exe/g;
...
export function setDesktopDownloadVersion(html, version, path) {
  ...
  return html
    .replace(MAC_DMG_RE, `cryptic-realm-${normalized}-mac-universal.dmg`)
    .replace(LINUX_APPIMAGE_RE, `cryptic-realm-${normalized}-linux-x86_64.AppImage`)
    .replace(WINDOWS_INSTALLER_RE, `cryptic-realm-${normalized}-win-x64.exe`)
    .replace(LEGACY_WINDOWS_INSTALLER_RE, `cryptic-realm-${normalized}-win-x64.exe`);
}
...
export function collectReleaseVersionFailures({
  ...
  const expectedArtifact = `cryptic-realm-${expected}-mac-universal.dmg`;
  const expectedLinuxArtifact = `cryptic-realm-${expected}-linux-x86_64.AppImage`;
  const expectedWindowsArtifact = `cryptic-realm-${expected}-win-x64.exe`;
