...
/**
 * The exact bytes `generate` writes, so tests/media_manifest_integrity.test.ts can
 * prove the committed artifact through this code path rather than a second copy of
 * the hashing and formatting rules — a re-implementation is one more thing that can
 * drift from the generator, which is the failure this whole guard exists to catch.
 */
...
export const GENERATED_PATH = generatedPath;

function generate() {
  const entries = manifestEntries();
  mkdirSync(path.dirname(generatedPath), { recursive: true });
  writeFileSync(generatedPath, renderManifest(entries));
  console.log(
...
// Importing this module must not run the CLI. Inside a vitest worker process.argv is
// exactly [node, forks.js], so argv[2] was undefined and the `?? 'generate'` default
// fired on import: tests/media_duplicate_prune.test.ts REGENERATED the tracked manifest
// on every suite run. That is what kept a stale committed manifest from ever failing a
// test — any run silently repaired the working copy while the commit stayed wrong, and
// it dirtied the worktree that scripts/realm_assets/deploy_all.sh then has to `checkout -f`.
