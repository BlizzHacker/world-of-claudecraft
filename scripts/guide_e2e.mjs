...
try {
  ...
  check('home hero renders', /Cryptic Realm/i.test(heroText), heroText);
  ...
  check(
    'focus moved to main',
    await page.evaluate(() => document.activeElement?.id === 'guide-main'),
  );
  ...
  check(
    ...
    (await page.$eval('.guide-class-hero-name', (el) => el.textContent.trim())).length > 0,
  );
  ...
  const healerVisible = await page.$$eval(
    '.guide-class-card',
    (els) => els.filter((e) => !e.hidden).length,
  );
  check(
    'chooser filters the grid',
    healerVisible > 0 && healerVisible < 9,
    `visible=${healerVisible}`,
  );
  ...
  for (const [path, sel] of [
    ...
    await page.waitForSelector('.guide-article h1');
    check(
      `new page renders: ${path}`,
      !!(await page.$(sel)) && !(await page.$('.guide-placeholder')),
    );
  }
  ...
  check(
    'table of contents present on a multi-section page',
    (await page.$$('.guide-toc a')).length >= 3,
  );
  ...
  check(
    ...
    await mobile.$eval('.guide-menu-toggle', (el) => getComputedStyle(el).display !== 'none'),
  );
