...
await new Promise(r=>setTimeout(r,4000));
for(let i=0;i<2;i++){ await p.screenshot({path:`/tmp/tr_idle${i}.png`}); await new Promise(r=>setTimeout(r,1500)); }
await p.keyboard.down('s');
for(let i=0;i<3;i++){ await new Promise(r=>setTimeout(r,1000)); await p.screenshot({path:`/tmp/tr_walk${i}.png`}); }
await p.keyboard.up('s');
await new Promise(r=>setTimeout(r,1500));
await p.keyboard.press('Tab');
await new Promise(r=>setTimeout(r,1000));
await p.keyboard.press('1');
for(let i=0;i<3;i++){ await new Promise(r=>setTimeout(r,700)); await p.screenshot({path:`/tmp/tr_atk${i}.png`}); }
await br.close();
console.log('done');
