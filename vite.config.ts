...
function i18nModulepreloadPlugin() {
  ...
  return {
    ...
    // writeBundle (not closeBundle): rolldown's native manifest writer and this JS
    // plugin bridge asynchronously, so closeBundle can fire before dist/.vite/
    // manifest.json is flushed to disk. writeBundle is the hook rollup/rolldown
    // guarantee runs only after every output file (including the manifest) is
    // physically written.
    writeBundle() {
...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      ...
      '/me/api': { target: apiProxyTarget, changeOrigin: true },
      '/mod/api': { target: apiProxyTarget, changeOrigin: true },
      '/forged': { target: apiProxyTarget, changeOrigin: true },
