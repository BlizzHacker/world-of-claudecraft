...
Break a large feature into a phased implementation plan designed for multiple Codex sessions. Every phase runs as its own fresh session and uses **Opus 4.8 at max effort** (`ultracode` where the phase warrants deterministic multi-agent orchestration). The whole point is to **save context per phase**: the orchestrator delegates reading and fan-out to subagents and keeps only conclusions, so each session stays sharp.
